...
function findCodes(str) {
    const arr = str.split(/\r?\n/);
    let sum = 0;
    const numberIdentifier = ["one", "two", "three", "four", "five", "six", "seven", "eight", "nine"];
    const sumarray = [];
    for(let i=0;i < arr.length; i++){
        let val = arr[i];
        for(let j=0;j < numberIdentifier.length; j++){
            val = val.replace(numberIdentifier[j], `${numberIdentifier[j]}${j+1}${numberIdentifier[j]}`)
        }
        // console.log(sum ,val.match(/\d{1}/)[0], val.match(/\d(?=\D*$)/)[0])
        sum += parseInt(val.match(/\d{1}/)[0] + val.match(/\d(?=\D*$)/)[0])
    }
    return sum;
}

console.log(findCodes(a));